    ...
    <form method="post" action="create_inc.php" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="inc_sev_id" name="inc_sev_id" required>
                ...
                <?php
                // Fetch incident severities from the database.  Modified to fetch from incident_severity
                $severity_query = "SELECT inc_sev_id, severity_name FROM incident_severity";
                $severity_result = $conn->query($severity_query);

                if (!$severity_result) {
                    die("Severity query failed: " . $conn->error);
                }
                ...
                if ($severity_result->num_rows > 0) {
                    ...
                    echo "<option value=''>No severities found</option>"; // Add this to handle the case where no severities are found
                }
                ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <small class="text-muted">Select all affected assets.</small>
        </div>
        <div class="form-group">
...
<?php
// Include the footer or any closing elements from your layout
